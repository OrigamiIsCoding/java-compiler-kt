 ...
 */
class GrammarAnalyzer(lines: List<String>) {
    private val productions: List<Production>
    private val firstSet: FirstSet
    private val followSet: FollowSet
    private val parsingTable: LLOneParsingTable
    ...
    init {
        ...
        followSet = if (firstSet.values.flatten().map { it.first }.contains(Symbol.Empty)) {
            FollowSet(firstSet, productions)
        } else {
            FollowSet.empty()
        }
        parsingTable = LLOneParsingTable(firstSet, followSet)
    }

    fun parse(sentence: String): List<Production> {
        // 输入的句子符号
